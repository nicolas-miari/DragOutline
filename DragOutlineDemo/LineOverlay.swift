...
//  DragOutlineDemo
//
//  Created by Rob Mayoff on 2017/04/28.
//
//
import AppKit
...
class LineOverlay {
    ...
    init(startScreenPoint: CGPoint, endScreenPoint: CGPoint) {
        ...
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(LineOverlay.screenLayoutDidChange(_:)),
    ...
    private func setViewPoints() {
        for window in windows {
            guard let view = window.contentView as? ConnectionView else {
                continue
            }
...
private class ConnectionView: NSView {
    ...
    override func layout() {
        ...
        shapeLayer.path = CGPath.barbell(
            from: parameters.startPoint,
            to: parameters.endPoint,
            barThickness: parameters.barThickness,
            bellRadius: parameters.ballRadius)

        shapeLayer.shadowPath = CGPath.barbell(
            from: parameters.startPoint,
            to: parameters.endPoint,
            barThickness: parameters.barThickness + shapeLayer.lineWidth / 2,
            bellRadius: parameters.ballRadius + shapeLayer.lineWidth / 2)
    }
...
private extension CGPath {
    class func barbell(
        ...
        path.addArc(
            center: CGPoint(x: length, y: 0),
            radius: bellRadius, startAngle: .pi + jointRadians, endAngle: .pi - jointRadians,
            clockwise: false)
        path.closeSubpath()
